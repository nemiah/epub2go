...
var multiCMS = {
	formHandler: function(formID, onSuccessFunction, onErrorFunction){
		if(!$j('#'+formID)) alert("Formular nicht gefunden!");
		
		$j.ajax({
			url: "/index.php?_="+Math.random(),
			data: "formID="+formID+"&"+$j('#'+formID).serialize(),
	...
	checkResponse: function(transport) {
		...
		if(transport.search(/^validate/) > -1){
			...
			$j("[name="+ex[0]+"]").addClass("errorField");
			$j("[name="+ex[0]+"]").parent().append("<div class=\"errorMessage\">"+ex[1]+"</div>");
			
			$j('html, body').animate({
				scrollTop: $j("[name="+ex[0]+"]").offset().top - 200
			}, 1000);
			
			return false;
		}
		if(transport.search(/Fatal error/) > -1){
			alert(transport.replace(/<br \/>/g,"\n").replace(/<b>/g,"").replace(/<\/b>/g,"").replace(/&gt;/g,">").replace(/^\s+/, '').replace(/\s+$j/, ''));
			return false;
		}
		return true;
	}
